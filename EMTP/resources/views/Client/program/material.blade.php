                    ...
                    @foreach($trainingMaterial as $indexKey => $material)
                        @if($material->program_code == $program->code)
                            <tr class="grid-cols-1 rounded shadow-sm m-2">
                                <td class="w-auto py-5 px-2"><h2 class="text-left pl-10">{{$material->title}}</h2></td>
                                <td class="w-4">
                                ...
                                <td class="w-4">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-4 mr-10 transform hover:text-purple-500 hover:scale-110">
