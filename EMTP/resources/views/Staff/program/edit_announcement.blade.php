<x-app-layout title="Staff Specific Program - Edit Announcement">
    <div class="bg-white sm:rounded-lg flex">
        ...
        <div class="block w-full min-h-screen">
            <form method="post" action="{{ route('staff-program-announcement-update', [$assignedprogram, $program, $announcement]) }}" class="w-auto m-5" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="w-auto mx-3 mb-6">
                    <div class="px-3">
                        <label class="text-gray-700 text-xl font-bold mb-2" for="title">Title</label>
                        <input id="title" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 focus:outline-none focus:bg-white focus:border-gray-500" type="text" name="title" value="{{$announcement->title}}" required autofocus autocomplete="name"/>
                    </div>
                </div>
                <div class="w-auto mx-3 mb-6">
                    <div class="px-3">
